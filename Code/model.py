...
def Resnet50_model(train_data, train_label, valid_data, valid_label, test_data, test_label):

	base_model = applications.ResNet50(weights= 'imagenet',include_top=False,input_shape=(256, 320, 3))
	...
	hist = model.fit(train_data,train_label,batch_size=16,epochs = nb_epochs,shuffle=True,verbose=1,validation_data=(valid_data, valid_label),
                 	callbacks=[ModelCheckpoint('ResNet-transferlearning2.model', monitor='val_acc', save_best_only=True)])

	#print ('test set outcome: ',model.evaluate(test_data,test_label,batch_size = 16))
	...
	#acc1,f1 = acc_and_f1('prediction.csv')
	predict_list = []
	for i,row in enumerate(test_label):
	...
	with open('Resnet50_prediction_integration.csv','w') as f:
